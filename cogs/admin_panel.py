...
class ModalDeleteWarn(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        if result_cheak_db: # Если пользователь есть в таблице тогда
            ...
            channel = interaction.guild.get_channel(await log_db.get_log_channel(interaction.guild))  # Вставить ID канала куда будут отправляться заявки
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        if result_cheak_db: # Если пользователь есть в таблице тогда
            if await user_db.get_user_warn_count(self.member.id) >= 3: # Проверяем количество предупреждений и кикаем если больше или равно 3
                ...
                channel = interaction.guild.get_channel(await log_db.get_log_channel(interaction.guild.id))  # Вставить ID канала куда будут отправляться заявки
            ...
            channel = interaction.guild.get_channel(await log_db.get_log_channel(interaction))  # Вставить ID канала куда будут отправляться заявки
...
class ModalMute(disnake.ui.Modal):
    ...
    async def callback(self, interaction: disnake.ModalInteraction) -> None:
        ...
        embed = disnake.Embed(color=0x2F3136, title="Пользователь замючен!")
        ...
        channel = interaction.guild.get_channel(
            await log_db.get_log_channel(interaction.guild))  # Вставить ID канала куда будут отправляться заявки
        await channel.send(
            f"Администратор {interaction.author.mention} изменил имя пользователю {self.member.mention} на {new_name} ( по причине: {reason})")
